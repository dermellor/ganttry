...
import { Badge } from '../../pluginHost/api';
...
export function newBadge(): HTMLElement {
  return Badge({ label: 'Neu', size: 'sm', caps: true, filled: true, className: 'pm-badge' });
}

/** An existing feature reworked in the pinned version. Secondary to „Neu". */
export function modifiedBadge(): HTMLElement {
  return Badge({ label: 'Modified', size: 'sm', caps: true, tone: 'accent', className: 'pm-badge' });
}
