...
// A range: the box spans the duration, so its right edge is its finish.
function range(left: number, right: number, midY: number): Anchor {
  return { left, right, top: midY - 12, bottom: midY + 12, midY, finishX: right, point: false };
}

// A milestone: `dotX` is the date, `right` is only where the caption happens to
// end. The gap between the two is what these tests are about.
