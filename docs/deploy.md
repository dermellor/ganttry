...
| `PLUGIN_ALLOWED_ORIGINS` | build + runtime       | comma-separated origins plugin code may be loaded from and talk to, added to `script-src` and `connect-src`. Empty (the default) means the page may only reach itself. Read at BUILD time into `_headers`, so changing it needs a redeploy; the API reads it too, and refuses an install from an origin that is not on the list rather than storing a plugin that can never load — see [`plugin-isolation.md`](plugin-isolation.md) and [`plugin-lifecycle.md`](plugin-lifecycle.md) |
