...
- **Stored backgrounds remain entries.** They fill their group behind ordinary
  bars, but can still be selected and edited. Timeline-level phase tints are
  generated separately and remain non-interactive.
...
- **Containment lives in `metadata.parent: "id"`.** The id of the item this one is part of, at most one — the umbrella item then renders as a summary bar above its children and can fold them away. Stored on the *child*, like `dependsOn`, so „one parent per item" is structural rather than a rule to enforce. See „Parent and children" (docs/items.md).
